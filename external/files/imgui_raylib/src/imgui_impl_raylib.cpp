...
static char* CodepointToUtf8(int codepoint, int* length)
{
    static char utf8[5];
    if (codepoint < 0x80) {
        utf8[0] = codepoint;
...
void ImGui_ImplRaylib_LoadDefaultFontAtlas()
{
    if (!g_UnloadAtlas) {
        ...
        TraceLog(LOG_INFO, "Font texture dimensions: %d x %d", width, height);

        Texture2D tex = LoadTextureFromImage(image);
        g_AtlasTexID = tex.id;
        io.Fonts->TexID = (void*)(intptr_t)g_AtlasTexID;
        ...
        if (g_AtlasTexID == 0) {
            ...
            TraceLog(LOG_INFO, "Font texture loaded successfully! Texture ID: %d", g_AtlasTexID);
        }
...
void ImGui_ImplRaylib_Render(ImDrawData* draw_data)
{
    ...
    TraceLog(LOG_INFO, "Binding font texture ID: %d", g_AtlasTexID);
    rlEnableTexture(g_AtlasTexID);
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                {
                    unsigned int ti = (unsigned int)(intptr_t)pcmd->TextureId;
                    if (ti != 0) {
                        TraceLog(LOG_INFO, "Using texture ID: %d", ti);
                        rlEnableTexture(ti);
                        ...
                        TraceLog(LOG_ERROR, "Invalid texture ID: %u", ti);
                    }
                }
            }
